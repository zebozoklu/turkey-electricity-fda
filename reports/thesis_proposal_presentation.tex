...
\begin{frame}{The Daily Load Curve: A Portrait of Economic Life}
  ...
  \begin{column}{0.44\textwidth}
    ...
    \end{block}
    \vspace{0.5em}
    \begin{alertblock}{Ramadan effect}
      During Ramadan, the entire shape shifts: iftar creates a massive
      evening spike, overnight demand rises sharply.
      The curve changes \emph{regime}, not just level.
    \end{alertblock}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Validation and Robustness}
  ...
  \begin{column}{0.48\textwidth}
    ...
    Replace OLS with RF on the same feature vector $z_d$
    (same FPCA structure, same rolling-origin design).
    Tests whether the linear score equation misses nonlinearity.
    \textit{[Result pending.]}
  \end{column}
  \end{columns}
\end{frame}
