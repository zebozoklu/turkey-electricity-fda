  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \begin{tikzpicture}[scale=0.88]
        ...
        \node[font=\small] at (3.15,0.60) {Training (expanding window)};
        ...
        \draw[-Stealth, thick, accentred]
              (6.3,-0.55) -- (8.4,-0.55)
              node[midway, below, font=\small] {1,096 days, rolling-origin};
      \end{tikzpicture}
      \end{center}
      \vspace{0.4em}
      \small FPCA and VAR re-estimated before each forecast day.\\
      Only data dated $<d$ used to forecast day $d$.
    \end{column}
...
\begin{frame}{Main Results}
  ...
  \vspace{0.2em}
  \footnotesize
  *** $p<0.001$ (DM test, pooled over 26,304 obs.). MAE in MWh/hour.
  \colorbox{wingreen}{Green} = beats official.
  \note{Walk the table top to bottom. The seasonal na\"{i}ve shows the floor --- the problem is hard. Pure VAR dynamics are worse than the official. The model turns positive only when holiday and load-shape features are added. Derivative features add another 2 points. Both gains are significant at near-zero p-value ($p \approx 10^{-135}$ and $10^{-163}$ respectively). The main model (bold row) is the baseline contribution; the residual correction row previews the next extension.}
\end{frame}
...
\begin{frame}{Where the Model Wins and Where It Loses}
  \begin{columns}
    ...
    \end{column}
  \end{columns}
  \note{The ramp-hour dip at hours 6--9 is the key diagnostic. Demand nearly doubles during this window as factories start and HVAC switches on. We do not know what gives the official forecast its advantage at ramp hours --- do not speculate. This gap motivates the residual correction step on the next slide.}
\end{frame}
...
\begin{frame}{Robustness Checks}
  \begin{columns}
    \begin{column}{0.60\textwidth}
      \includegraphics[width=\linewidth]{../output/figs/direct_RF_mae_by_hour_notemp.png}
    \end{column}
    \begin{column}{0.37\textwidth}
      \textbf{Direct RF vs.\ FPCA-OLS (2023):}
      \vspace{0.3em}

      \small
      \begin{itemize}
        \item Direct RF (no FPCA) wins at most daytime hours (8--23)
        \item FPCA-OLS wins overnight (0--5)
        \item Neither model uses temperature data
      \end{itemize}

      \vspace{0.5em}
      \textbf{$K=4$ components:}
      \small
      \begin{tabular}{lrr}
        \toprule
        $K$ & MAE & MAE (+resid.) \\
      ...
      \small $K=2$ is the parsimonious baseline.
    \end{column}
  \end{columns}
  \note{Direct RF (forecasting each hour separately, no FPCA) beats FPCA-OLS at most daytime hours --- this is a genuine finding. Do not downplay it. The FPCA-OLS advantage is at overnight hours. The right framing: both approaches beat the official; the comparative strengths differ by time of day. Note: the figure also shows a temperature variant of Direct RF --- the two lines are nearly identical, so focus attention on the no-temperature line.}
\end{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item[$\square$] Weather/temperature covariates (ramp hours)
        \item[$\square$] Probabilistic forecasting (prediction intervals)
        \item[$\square$] Thesis write-up
      \end{itemize}
    \end{column}
  \end{columns}
  \note{The empirical core is complete. Open items: (1) a formal literature review situating this within FDA and electricity forecasting, (2) adding temperature data to address the ramp-hour weakness, and (3) probabilistic extensions if time allows. Be direct with the committee about what is done vs planned.}
\end{frame}
