...
(a) model staleness in a period of growing demand, and (b) asymmetric
institutional loss functions favouring under-procurement.
The growing gap in 2024 is most naturally explained by the model not
adapting to a structural demand increase compounded by an unusually hot summer.
\end{intuitionbox}
...
public holidays cause sharp drops,
...
days --- during holiday periods the ramp shifts to a different pattern and
persists for days; during summer the ramp is shallower and this
...
\textbf{FPCA-RF vs FPCA-OLS (full 2023--2025 evaluation).}
Using the same feature vector $z_d$ plus lagged scores as inputs to a
random forest (one RF model per FPCA score, same rolling-origin design,
FPCA basis fixed on the expanding training window):

\begin{center}
\begin{tabular}{lrrrrr}
\toprule
Model & MAE & MAPE & MAE gain & Conclusion \\
\midrule
Official forecast & 1214 & 3.15\% & 0\% & benchmark \\
FPCA-RF (fixed basis) & 1217 & 3.14\% & $-0.3\%$ & ties official \\
FPCA-OLS (main model) & \textbf{980} & \textbf{2.56\%} & $+19.2\%$ & best \\
\bottomrule
\end{tabular}
\end{center}

The outcome is the first case: \textbf{FPCA-OLS substantially outperforms FPCA-RF}.
RF achieves essentially the same accuracy as the official forecast (MAE 1217 vs 1214)
while OLS beats the official by 19\%. RF finds no meaningful nonlinearity to exploit:
the linear score equation is adequate, and the VAR structure plus linear feature
mapping is actually beneficial as a regulariser.

\begin{resultbox}
\textbf{Conclusion:} The FPCA scores are approximately linearly predictable from the
features. OLS's linear structure reduces variance without sacrificing fit; RF's
additional flexibility adds variance without a compensating reduction in bias.
This validates the linear score regression and the FDA/VAR model architecture.
\end{resultbox}

\textbf{Direct RF as an additional exercise (2025 subsample only).}
A ``direct RF'' model that skips the FPCA structure entirely and predicts each of the
24 hourly values directly (using the same features plus lagged hourly values) achieves
MAE = 718 on the 2025 subsample, a 31.8\% gain over the official (MAE 1052) in that
year, compared to FPCA-OLS's 13.5\% gain (MAE 910) in the same period.
A direct ML approach can therefore be competitive, but the comparison is on a single
year and does not use the functional structure. The FPCA model's 19.2\% gain over the
full three-year period --- with interpretable eigenfunctions, a testable stability
condition, and a clear decomposition of the forecast --- remains the primary result.
...
\begin{enumerate}
  ...
  \item \textbf{The benchmark:} FPCA-RF (fixed basis) achieves MAE = 1217,
    essentially tying the official forecast, while FPCA-OLS achieves MAE = 980.
    The linear score equation is adequate; RF finds no nonlinearity to exploit.
