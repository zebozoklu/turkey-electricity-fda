...
# ------------------------------------------------------------
# One-step-ahead FPCA VAR(1,7) on scores
# ------------------------------------------------------------

forecast_one_day_var_1_7 <- function(i, Y, hours, K, nbasis, norder) {
  
  # training sample: only days before target day i
  ...
  # model:
  # xi_d = c + A1 xi_{d-1} + A7 xi_{d-7} + e_d
  #
  # usable training days: d = 8,...,n_train
  
  Y_score <- scores[8:n_train, , drop = FALSE]
  
  X_lag1 <- scores[7:(n_train - 1), , drop = FALSE]
  X_lag7 <- scores[1:(n_train - 7), , drop = FALSE]
  ...
  # forecasting target day i:
  # lag 1 = day i-1 = training row n_train
  # lag 7 = day i-7 = training row n_train - 6
...
eval_df <- bind_rows(
  ...
  make_eval_df(actual_test, pred_var_1_7, "FPCA VAR(1,7) scores")
)
...
write.csv(
  metric_table,
  "output/tables/fpca_VAR_1_7_metrics_overall.csv",
  row.names = FALSE
)
...
write.csv(
  metrics_by_year,
  "output/tables/fpca_VAR_1_7_metrics_by_year.csv",
  row.names = FALSE
)
...
write.csv(
  metrics_by_hour,
  "output/tables/fpca_VAR_1_7_metrics_by_hour.csv",
  row.names = FALSE
)
...
ggsave(
  "output/figs/fpca_VAR_1_7_mae_by_hour.png",
  p_hour,
  width = 8,
  height = 4.5
)
...
example_df <- tibble(
  ...
  `FPCA VAR(1,7)` = pred_var_1_7[example_day, ]
)
...
ggsave(
  "output/figs/fpca_VAR_1_7_example_curve.png",
  p_example,
  width = 8,
  height = 4.5
)
...
saveRDS(
  forecast_objects,
  "output/results/fpca_VAR_1_7_forecast_results.rds"
)
